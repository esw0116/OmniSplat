...
def combine_Gaussian(gs_lst:list) -> Gaussians:
    mean, cov, harmonic, opacity = [], [], [], []
    ...
    return Gaussians(mean, cov, harmonic, opacity)


def combine_visdump(vis_list):
    depths, scales, rotations = [], [], []
    for visdump in vis_list:
        # depths.append(visdump["depth"])
        scales.append(visdump["scales"])
        rotations.append(visdump["rotations"])
        
    # depths = torch.cat(depths, dim=1)
    scales = torch.cat(scales, dim=1)
    rotations = torch.cat(rotations, dim=1)
    
    return {"depth": depths, "scales": scales, "rotations": rotations}
