...
class EncoderCostVolumeCfg:
    name: Literal["costvolume", "crosscostvolume", "priorcrosscostvolume", "concatcostvolume", "priorconcatcostvolume"]
    d_feature: int
...
class EncoderCostVolume(Encoder[EncoderCostVolumeCfg]):
    ...
    def forward(
        ...
        mode: Optional[str] = "omni",
        omnidirectional: Optional[str] = "",
    ) -> Gaussians:
        
        
        if mode in ["omni", "sepyy"]:
            image_key = "image"
            intrinsics_key = "intrinsics"
        else:
            image_key = "image_" + mode
            intrinsics_key = "intrinsics_" + mode
        ...
        device = context[image_key].device
        b, v, _, h, w = context[image_key].shape
        ...
        if self.cfg.use_epipolar_trans:
            epipolar_kwargs = {
                ...
                "intrinsics": context[intrinsics_key],
                "near": context["near"],
                "far": context["far"],
            }
        else:
        ...
        gaussians = self.gaussian_adapter.forward(
            ...
            rearrange(context[intrinsics_key], "b v i j -> b v () () () i j"),
