...
from ..backbone.unimatch.geometry import coords_grid, omni_to_3d, yin_to_3d, yang_to_3d, yang90_to_3d, omni_from_3d, yin_from_3d, yang_from_3d, yang90_from_3d
from .ldm_unet.unet import UNetModel


def omni_warp_with_pose_depth_candidates(
    feature1,
    intrinsics,
    pose,
    depth,
    ...
    with torch.no_grad():
        ...
        if omni_mode=='omni':
            points = omni_to_3d(grid.view(b, 3, -1), h, w)   # [B, 3, H*W]
        elif omni_mode=='yin':
            points = yin_to_3d(grid.view(b, 3, -1), h, w)   # [B, 3, H*W]
        elif omni_mode=='yang':
            points = yang_to_3d(grid.view(b, 3, -1), h, w)   # [B, 3, H*W]
        elif omni_mode=='yang90':
            points = yang90_to_3d(grid.view(b, 3, -1), h, w)   # [B, 3, H*W]
        else:
            raise NotImplementedError('omnidirectional mode should be one of [omni, yin, yang, yang90]')
        ...
        if omni_mode=='omni':
            points = omni_from_3d(points, h, w)
        elif omni_mode=='yin':
            points = yin_from_3d(points, h, w)
        elif omni_mode=='yang':
            points = yang_from_3d(points, h, w)
        elif omni_mode=='yang90':
            points = yang90_from_3d(points, h, w)
    ...
    def forward(
        ...
        if self.wo_cost_volume:
            ...
            for feat10, pose_curr in zip(feat_comb_lists[1:], pose_curr_lists):
                ...
                if omnidirectional in ['omni', 'yin', 'yang']:
                    feat01_warped = omni_warp_with_pose_depth_candidates(
                        feat10,
                        intr_curr,
                        pose_curr,
                ...
                elif omnidirectional in ['sepyy']: # First half: yin, Last half: yang90
                    feat01_warped = omni_warp_with_pose_depth_candidates(
                        feat10,
                        intr_curr,
                        pose_curr,
