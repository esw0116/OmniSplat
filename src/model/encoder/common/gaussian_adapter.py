...
from ....geometry.projection import get_omni_world_rays, get_world_rays
...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        if omnidirectional in ['yin', 'yang', 'omni']:
            # identity_intrinsics = torch.eye(3).unsqueeze(0).repeat(intrinsics.shape[0], 1, 1).type_as(intrinsics)
            identity_intrinsics = repeat(torch.eye(3), 'x y -> b v p q r x y', b=intrinsics.shape[0], v=intrinsics.shape[1], p=1, q=1, r=1).type_as(intrinsics)
            multiplier = self.get_scale_multiplier(identity_intrinsics, pixel_size)
        else:
        ...
        if omnidirectional in ['omni', 'yin', 'yang', 'yang90']:
            origins, directions = get_omni_world_rays(coordinates, extrinsics, intrinsics, omnidirectional)
        else:
