	...
	bool prefiltered)
{
	...
	float dist_yz = sqrt(p_view.y*p_view.y + p_view.z*p_view.z)+0.0000001f;
	float dist_xz = sqrt(p_view.x*p_view.x + p_view.z*p_view.z)+0.0000001f;
	float lat = atan2(-p_view.x, dist_yz);
	float lon = atan2(p_view.y, -p_view.z);
	float2 p_omniuv = {0.0*lon + 2*(W-1)/M_PI*lat + (W/2-0.5), 2*(H-1)/3/M_PI*lon + 0.0*lat + (H/2-0.5)};
	...
	float3 cov = computeOmniCov2D(p_orig, focal_x, focal_y, lon, lat, cov3D, viewmatrix, H, W);
	...
	float* __restrict__ out_acc)
{
	...
	if (inside)
	{
		...
		A += 1e-8;
		out_depth[pix_id]= 1 / (D+0.000001f);
        out_acc[pix_id] = A;
	}
}
