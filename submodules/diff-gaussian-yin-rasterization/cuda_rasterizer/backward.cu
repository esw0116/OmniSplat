	...
	float* dL_dcov)
{
	...
	const float x_scale = 2*(width-1)/(3*M_PI);
	const float y_scale = 2*(height-1)/(M_PI);
	...
	glm::vec4* dL_drot)
{
	...
	dL_dmean.x = (2*(W-1)/3/M_PI) * ( (viewmatrix[0]*m_view.z - viewmatrix[2]*m_view.x) / (dist_xz * dist_xz)) * dL_dmean2D[idx].x + (-2*(H-1)/M_PI) * ( (-viewmatrix[1]*dist_xz*dist_xz + (viewmatrix[0]*m_view.x+viewmatrix[2]*m_view.z)*m_view.y ) / (dist * dist * dist_xz)) * dL_dmean2D[idx].y;
	dL_dmean.y = (2*(W-1)/3/M_PI) * ( (viewmatrix[4]*m_view.z - viewmatrix[6]*m_view.x) / (dist_xz * dist_xz)) * dL_dmean2D[idx].x + (-2*(H-1)/M_PI) * ( (-viewmatrix[5]*dist_xz*dist_xz + (viewmatrix[4]*m_view.x+viewmatrix[6]*m_view.z)*m_view.y ) / (dist * dist * dist_xz)) * dL_dmean2D[idx].y;
	dL_dmean.z = (2*(W-1)/3/M_PI) * ( (viewmatrix[8]*m_view.z - viewmatrix[10]*m_view.x) / (dist_xz * dist_xz)) * dL_dmean2D[idx].x + (-2*(H-1)/M_PI) * ( (-viewmatrix[9]*dist_xz*dist_xz + (viewmatrix[8]*m_view.x+viewmatrix[10]*m_view.z)*m_view.y ) / (dist * dist * dist_xz)) * dL_dmean2D[idx].y;
